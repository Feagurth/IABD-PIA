...
def leer_entero(minimo:int, maximo:int, mensaje:str):
    ...
    while(True):        
        try:
            ...
            # Si tenemos una excepción seguimos iterando el bucle
            continue
...
def reset_juego():
    ...
    cls()
    Datos_programa.tablero =  [[" " for count in range(3)] for count in range(3)]
...
def pintar_color(texto:str, color:type, retorno:bool = False)->str:
    ...
    # dependiendo de esto
    if(Datos_programa.colores_activos):
        if(retorno):
            print(f"{color}" + texto + f"{Datos_programa.RESET}")
        else:
            print(f"{color}" + texto + f"{Datos_programa.RESET}", end="")
    else:
        if(retorno):
            print(texto)
        else:
...
def pintar_tablero():
    ...
    for x in Datos_programa.tablero:
        ...
        for y in range(0,3):
            ...
            if(x[y]) == "X":
                pintar_color("X", Datos_programa.AMARILLO)
        
            if(x[y]) == "O":
                pintar_color("O", Datos_programa.ROJO)
...
def mostrar_tablero_info(opcion_menu):
    ...
    pintar_color("X", Datos_programa.AMARILLO, True)
    print("El " + ("Jugador 2" if (opcion_menu == 2) else "Ordenador") + " juega con ", end="")
    pintar_color("O", Datos_programa.ROJO, True)
    print("")
...
def comprobar_fila(fila:int, simbolo:str)->bool:
    ...
    for x in range(0,3):
        if(not Datos_programa.tablero[fila][x] == simbolo):
            return False
...
def comprobar_columna(columna:int, simbolo:str)->bool:
    ...
    for x in range(0,3):
        if(not Datos_programa.tablero[x][columna] == simbolo):
            return False
...
def comprobar_diagonal(simbolo:str)->bool:
    ...
    if(Datos_programa.tablero[0][0] == simbolo and Datos_programa.tablero[1][1] == simbolo and Datos_programa.tablero[2][2] == simbolo):
        return 1
    elif(Datos_programa.tablero[0][2] == simbolo and Datos_programa.tablero[1][1] == simbolo and Datos_programa.tablero[2][0] == simbolo):
         return 1
    else:
        return 0
...
def comprobar_ganador()->int:
    ...
    if(not any(" " in sub for sub in Datos_programa.tablero)):
        return 0
